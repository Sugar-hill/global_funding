...
export type StakingContract$Data = {
    $$type: 'StakingContract$Data';
    id: bigint;
    counter: bigint;
}

export function storeStakingContract$Data(src: StakingContract$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadStakingContract$Data(slice: Slice) {
    const sc_0 = slice;
    const _id = sc_0.loadUintBig(32);
    const _counter = sc_0.loadUintBig(32);
    return { $$type: 'StakingContract$Data' as const, id: _id, counter: _counter };
}

function loadTupleStakingContract$Data(source: TupleReader) {
    const _id = source.readBigNumber();
    const _counter = source.readBigNumber();
    return { $$type: 'StakingContract$Data' as const, id: _id, counter: _counter };
}

function loadGetterTupleStakingContract$Data(source: TupleReader) {
    const _id = source.readBigNumber();
    const _counter = source.readBigNumber();
    return { $$type: 'StakingContract$Data' as const, id: _id, counter: _counter };
}

function storeTupleStakingContract$Data(source: StakingContract$Data) {
    const builder = new TupleBuilder();
...
function dictValueParserStakingContract$Data(): DictionaryValue<StakingContract$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeStakingContract$Data(src)).endCell());
        },
        parse: (src) => {
            return loadStakingContract$Data(src.loadRef().beginParse());
        }
    }
}
...
async function StakingContract_init(id: bigint) {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initStakingContract_init_args({ $$type: 'StakingContract_init_args', id })(builder);
...
const StakingContract_types: ABIType[] = [
    ...
    {"name":"StakingContract$Data","header":null,"fields":[{"name":"id","type":{"kind":"simple","type":"uint","optional":false,"format":32}},{"name":"counter","type":{"kind":"simple","type":"uint","optional":false,"format":32}}]},
]
...
export const StakingContract_getterMapping: { [key: string]: string } = {
    'counter': 'getCounter',
    'id': 'getId',
}
...
export class StakingContract implements Contract {
    ...
    static async init(id: bigint) {
        return await StakingContract_init(id);
    }
    
    static async fromInit(id: bigint) {
        const __gen_init = await StakingContract_init(id);
        const address = contractAddress(0, __gen_init);
        return new StakingContract(address, __gen_init);
    }
